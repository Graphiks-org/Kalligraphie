...
- The bounded SVG-in-OpenType subset now accepts static `defs`/`linearGradient`/`stop` definitions applied to `rect`, normalizing local gradients with strict SVG numbers, unpremultiplied alpha interpolation, complete repeat/reflect periods, and reached-stop accounting into schema-3 `LinearGradient` or `Solid` paint under a rectangle `PathClip`; schema-1 solid SVG keeps its historical node/depth policy without a paint-visit budget.
