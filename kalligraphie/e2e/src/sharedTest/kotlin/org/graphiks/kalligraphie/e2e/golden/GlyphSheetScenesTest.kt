...
class GlyphSheetScenesTest {
    ...
    fun rendersATwoGlyphLatinSheet() {
        val first = GlyphSheetScenes.outlineSheet(E2eTestEnvironment.corpus, LIBERATION_SANS, listOf(0x41, 0x42), 32.0)
        assertEquals(PixelFormat.ALPHA_8, first.format)
        assertTrue(first.width > 0 && first.height > 0, "a sheet must contain pixel data")
        val second = GlyphSheetScenes.outlineSheet(E2eTestEnvironment.corpus, LIBERATION_SANS, listOf(0x41, 0x42), 32.0)
    ...
    fun failsWhenACodepointHasNoGlyph() {
        val failure = assertFailsWith<IllegalStateException> {
            GlyphSheetScenes.outlineSheet(E2eTestEnvironment.corpus, LIBERATION_SANS, listOf(0x10FFFF), 32.0)
        }
    ...
    fun rendersAnEmojiSheetFromThePaintRoute() {
        val image = GlyphSheetScenes.paintSheet(E2eTestEnvironment.corpus, EMOJI_TWO_COLR_V0, listOf(0x1F600, 0x1F601), 64.0, 0)
    ...
    @Test
    fun wrapsAcrossRows() {
        val oneRow = GlyphSheetScenes.outlineSheet(E2eTestEnvironment.corpus, LIBERATION_SANS, (0x41..0x50).toList(), 32.0)
        val twoRows = GlyphSheetScenes.outlineSheet(E2eTestEnvironment.corpus, LIBERATION_SANS, (0x41..0x50).toList() + 0x52, 32.0)
    ...
    fun paintSheetIsDeterministic() {
        val first = GlyphSheetScenes.paintSheet(E2eTestEnvironment.corpus, EMOJI_TWO_COLR_V0, listOf(0x1F600, 0x1F601), 64.0, 0)
        val second = GlyphSheetScenes.paintSheet(E2eTestEnvironment.corpus, EMOJI_TWO_COLR_V0, listOf(0x1F600, 0x1F601), 64.0, 0)
        assertContentEquals(first.copyCanonicalBytes(), second.copyCanonicalBytes())
    }
