 ...
 * is applied after normalization. An unknown axis tag fails with `font.variation.unknown-axis`.
 * The returned coordinates are tag-sorted, which satisfies `FontGeometryParameters`.
 */
...
public object VariationNormalizer {
    ...
    public fun normalize(
        ...
        for (coordinate in design.coordinates) {
            val axis = fvar.axis(coordinate.tag)
            ...
            val mapped = applyAvar(normalized, avar, fvar.axes.indexOf(axis))
