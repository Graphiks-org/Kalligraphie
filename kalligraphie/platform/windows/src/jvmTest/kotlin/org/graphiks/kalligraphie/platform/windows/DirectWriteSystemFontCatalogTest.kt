...
/**
 * Controlled-boundary tests for the DirectWrite system provider.
 *
 * The production provider enumerates the real DirectWrite system collection
 * through `kffi-directwrite`; these tests substitute a deterministic registry
 * over audited fixture files so discovery, refresh and cross-generation refusal
 * are verified without depending on what is installed on the machine.
 */
class DirectWriteSystemFontCatalogTest {
    @Test
    fun capturesExactlyTheControlledRegistrySet() {
        withFixtures { liberation, collection ->
            ...
            val catalog = success(DirectWriteSystemFontCatalog.open(registry = registry))
            ...
            assertEquals("directwrite-registry", catalog.generation.provider.value)
    ...
    fun refreshMintsANewGenerationWithoutInvalidatingThePreviousSnapshot() {
        withFixtures { liberation, collection ->
            val first = success(DirectWriteSystemFontCatalog.open(registry = registryOf(liberation to "Liberation Sans")))
    ...
    fun refusesAnEmptyRegistryWithATypedError() {
        val result = DirectWriteSystemFontCatalog.open(registry = DirectWriteFontRegistry { emptyList() })
    ...
    private fun registryOf(vararg files: Pair<Path, String>): DirectWriteFontRegistry = DirectWriteFontRegistry {
        files.map { (path, family) -> DirectWriteRegisteredFont(family, "Regular", path.toString(), family) }
    }

    private fun withFixtures(block: (liberation: Path, collection: Path) -> Unit) {
        val root = Files.createTempDirectory("directwrite-registry")
        try {
