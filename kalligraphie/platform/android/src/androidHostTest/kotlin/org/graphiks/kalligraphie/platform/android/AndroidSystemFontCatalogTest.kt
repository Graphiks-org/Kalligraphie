...
/**
 * Controlled-boundary tests for the Android system provider.
 *
 * The production provider enumerates the real platform collection through
 * `SystemFonts`; these tests substitute a deterministic registry over audited
 * fixture files so discovery, refresh and cross-generation refusal are verified
 * on the host JVM without a device.
 */
class AndroidSystemFontCatalogTest {
    @Test
    fun capturesExactlyTheControlledRegistrySet() {
        withFixtures { liberation, collection ->
            ...
            val catalog = success(AndroidSystemFontCatalog.open(registry = registry))
            ...
            assertEquals("android-platform-fonts", catalog.generation.provider.value)
    ...
    fun refreshMintsANewGenerationWithoutInvalidatingThePreviousSnapshot() {
        withFixtures { liberation, collection ->
            val first = success(AndroidSystemFontCatalog.open(registry = registryOf(liberation to "Liberation Sans")))
    ...
    fun refusesAnEmptyRegistryWithATypedError() {
        val result = AndroidSystemFontCatalog.open(registry = AndroidSystemFontRegistry { emptyList() })
    ...
    private fun registryOf(vararg files: Pair<Path, String>): AndroidSystemFontRegistry = AndroidSystemFontRegistry {
        files.map { (path, _) -> AndroidRegisteredFont(path.toString()) }
    }

    private fun withFixtures(block: (liberation: Path, collection: Path) -> Unit) {
        val root = Files.createTempDirectory("android-registry")
        try {
