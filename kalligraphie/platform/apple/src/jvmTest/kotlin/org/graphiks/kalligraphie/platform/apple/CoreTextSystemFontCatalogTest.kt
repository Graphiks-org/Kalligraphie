...
/**
 * Controlled-boundary tests for the CoreText system provider.
 *
 * The production provider enumerates the real CoreText registry through
 * `kffi-coretext`; these tests substitute a deterministic registry over audited
 ...
 */
class CoreTextSystemFontCatalogTest {
    @Test
    fun capturesExactlyTheControlledRegistrySet() {
        withFixtures { liberation, collection ->
            ...
            val catalog = success(CoreTextSystemFontCatalog.open(registry = registry))

            val families = catalog.faces.map { it.id.source }.toSet()
            ...
            assertEquals("coretext-registry", catalog.generation.provider.value)
            assertTrue(families.all { it is FontSourceId.Portable }, "captured sources must be portable")
        }
    }
    ...
    fun refreshMintsANewGenerationWithoutInvalidatingThePreviousSnapshot() {
        withFixtures { liberation, collection ->
            val first = success(CoreTextSystemFontCatalog.open(registry = registryOf(liberation to "Liberation Sans")))
    ...
    fun refusesAnEmptyRegistryWithATypedError() {
        val result = CoreTextSystemFontCatalog.open(registry = CoreTextFontRegistry { emptyList() })
    ...
    private fun registryOf(vararg files: Pair<Path, String>): CoreTextFontRegistry = CoreTextFontRegistry {
        files.map { (path, family) -> CoreTextRegisteredFont(family, "Regular", family, path.toString()) }
    }

    private fun withFixtures(block: (liberation: Path, collection: Path) -> Unit) {
        val root = Files.createTempDirectory("coretext-registry")
        try {
