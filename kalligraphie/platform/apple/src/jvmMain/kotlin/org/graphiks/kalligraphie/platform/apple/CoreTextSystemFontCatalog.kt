...
/** One font descriptor reported by the platform's activated font registry. */
public data class CoreTextRegisteredFont(
    /** Localized family name reported by the registry. */
    public val familyName: String,
    /** Localized style name reported by the registry. */
    public val styleName: String,
    /** PostScript name reported by the registry. */
    public val postScriptName: String,
    /** Absolute file path backing the descriptor, or empty when unknown. */
 ...
 *
 * Production uses [KffiCoreTextFontRegistry]; tests supply a controlled
 ...
 * caller state.
 */
public fun interface CoreTextFontRegistry {
    /** Returns the fonts the platform currently reports as activated. */
    public fun availableFonts(): List<CoreTextRegisteredFont>
}

/**
 * Registry backed by `kffi-coretext`'s `CoreTextFontCatalog`.
 *
 * Construction loads the CoreText/CoreFoundation/CoreGraphics frameworks and may
 * throw; [CoreTextSystemFontCatalog.open] converts that into a typed failure
 * rather than leaking an exception.
 */
public class KffiCoreTextFontRegistry : CoreTextFontRegistry {
    private val catalog = org.graphiks.kffi.coretext.CoreTextFontCatalog()

    override fun availableFonts(): List<CoreTextRegisteredFont> =
        catalog.availableFonts().map { font ->
            CoreTextRegisteredFont(font.familyName, font.styleName, font.postScriptName, font.filePath)
        }
}

/** Immutable positive bounds for one CoreText-registry capture. */
public class CoreTextSystemFontCatalogOptions(
...
/**
 * Faithful macOS system-font provider.
 *
 * Family and face discovery comes from the platform's **activated registry** —
 * the set CoreText would use, not a conventional directory listing — while the
 * portable data remains the captured OpenType bytes. Each success owns a fresh
 * `coretext-registry` generation and detached captured bytes, so later
 * installation or removal cannot change an earlier snapshot. Callers close their
 ...
 */
public object CoreTextSystemFontCatalog {
    private val generationCounter = AtomicLong()

    /**
     * Enumerates the activated registry and captures the portable catalogue.
     *
     ...
     */
    public fun open(
        options: CoreTextSystemFontCatalogOptions = CoreTextSystemFontCatalogOptions(),
        cancellationToken: CancellationToken = CancellationToken.none,
        registry: CoreTextFontRegistry? = null,
    ): FontOperationResult<FontCatalogSnapshot> {
        if (cancellationToken.isCancellationRequested()) return FontOperationResult.Cancelled()
        val sourceRegistry = registry ?: try {
            KffiCoreTextFontRegistry()
        } catch (failure: Throwable) {
            return failure(
                "font.coretext.registry-unavailable",
        ...
        val paths = registered.map(CoreTextRegisteredFont::filePath).filter(String::isNotBlank).distinct().sorted()
        if (paths.isEmpty()) {
            return failure("font.coretext.no-registered-fonts", "The CoreText registry reported no font files.")
        }
        ...
        for (path in paths) {
            ...
            if (!Files.isRegularFile(file, NOFOLLOW_LINKS)) {
                diagnostics.add("font.coretext.registered-file-missing", "Registered font file is absent or not a regular file.")
                continue
            }
        ...
        if (entries.isEmpty()) {
            return failure("font.coretext.no-capturable-fonts", "No registered font face could be captured.")
        }
        val generation = FontCatalogGeneration(
            FontProviderId("coretext-registry"),
            "coretext-${generationCounter.incrementAndGet()}",
        )
    ...
    private fun readBounded(
        ...
        diagnostics.add("font.coretext.registered-file-unreadable", "Registered font file could not be read.")
        null
    }
