...
/**
 * Opens the real iOS HarfBuzz platform binding over the published kffi iOS artifact.
 *
 * The Kotlin/Native declarations of the kffi HarfBuzz surface are identical to the JVM ones, so
 * [IosHarfBuzzPlatformBinding] mirrors `HarfBuzzBindings.jvm.kt` behaviour exactly: the same
 * design-unit-only scaling, the same total failure mapping and the same font→face→blob ownership
 * order. The matching `iosMain` actual in `HarfBuzzBindings.ios.kt` calls this function.
 */
internal fun openIosHarfBuzzPlatformBinding(): HarfBuzzPlatformBinding = try {
    IosHarfBuzzPlatformBinding(HarfBuzz.open())
...
private class IosHarfBuzzPlatformBinding(private val hb: HarfBuzz) : HarfBuzzPlatformBinding {
    ...
    override fun release(prepared: PlatformPreparedFont) {
        val ios = prepared as IosPreparedFont
        val failures = buildList {
            runCatching { ios.font.close() }.exceptionOrNull()?.let(::add)
            runCatching { ios.face.close() }.exceptionOrNull()?.let(::add)
            runCatching { ios.blob.close() }.exceptionOrNull()?.let(::add)
        }
        aggregateFailures(failures)?.let { throw it }
    }
}

private class IosPreparedFont(
...
private class IosHarfBuzzBuffer(
    ...
    override fun shape(prepared: PlatformPreparedFont, features: List<OpenTypeFeature>): Boolean {
        ...
        return buffer.shape(ios.font, kffiFeatures)
    }
