...
/**
 * The analyzer's behavioural suite, independent of any platform: the same cases the JVM reference
 * pins against ICU, pinned here against the generated tables, so every target that compiles this
 * test answers for the analysis its own binary would carry out.
 */
class UcdUnicodeAnalyzerTest {
    @Test
    fun extended_grapheme_cluster_keeps_emoji_zwj_sequence_together() {
        ...
        assertEquals("Kalligraphie", analysis.unicodeData.implementation)
        assertEquals("16.0", analysis.unicodeData.implementationVersion)
    }
    ...
    @Test
    fun scalar_budget_is_enforced_before_any_analysis_runs() {
        val snapshot = snapshotOf("abc")

        val outcome = analyzer.analyze(
            snapshot,
            UnicodeAnalysisRequest(BaseDirection.LEFT_TO_RIGHT, language = "en"),
            UnicodeAnalysisProfile(maxScalars = 2),
            CancellationToken.none,
        )

        val limit = assertIs<UnicodeAnalysisOutcome.LimitExceeded>(outcome)
        assertEquals(UnicodeAnalysisLimit.SCALARS, limit.limit)
        assertEquals(3, limit.observed)
    }

    @Test
    fun already_cancelled_token_publishes_nothing() {
        val snapshot = snapshotOf("abc")
        ...
        assertEquals(UnicodeAnalysisOutcome.Cancelled, outcome)
    }
    ...
    private companion object {
        val analyzer: BoundedUnicodeAnalyzer = PortableUnicodeAnalyzer.create()
    }
}
